...
beauties_raw <- beauties_raw %>% 
  select(genotype, rhtmajor, rhtminor, 9:17) %>% 
  group_by(genotype, rhtmajor, rhtminor) %>% 
  summarise_all(mean)

beauties_raw <- beauties_raw %>% 
  mutate(group = paste(genotype, rhtmajor, rhtminor, sep = "_"))

flowerplot <- beauties_raw %>% 
  filter(!genotype %in% c("C6", "Sceptre", "Shield")) %>% 
  ggplot(aes(x = reorder(group, Z20170926), y = Z20170926)) +
  geom_point()+
  facet_wrap(~genotype, scales = "free_x", nrow = 1)+
   theme_classic() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, size = 8)) +
   labs(title="Flowering time", x = "QTL combination", y = "Zadoks Score")
